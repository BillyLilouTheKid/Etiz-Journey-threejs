...
export default class Hitbox {
    ...
    activate(delay=0, damageData) {
        if (this.mode === HitboxModeEnum.Active) {
            setTimeout(()=>{
                GameObserver.getHandlers("Hitbox").fire(this, damageData);
            },delay);
        }
    }

    lift() {
        if (this.mode === HitboxModeEnum.Active) return GameObserver.getHandlers("Hitbox").returnIntersectEntity(this);
    }
}