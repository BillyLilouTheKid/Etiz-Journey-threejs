...
export default class EnemyController {
    ...
    tick() {
        const dx = this.target.position.x - this.body.position.x; // we get the x distance between the player position and the entity itself
        const dz = this.target.position.z - this.body.position.z; // we get the z distance between the player position and the entity itself
        const targetAngle = Math.atan2(dx, dz); // we get the angle between the dx and dz
